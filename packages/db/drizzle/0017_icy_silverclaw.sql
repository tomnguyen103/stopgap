...
--
-- RENUMBERED FROM 0015 TO 0017 on the ticket-16 merge. Two branches independently generated an
-- 0015; whichever lands second has to move, and the file was REGENERATED against the merged schema
-- rather than renamed, so the snapshot chain drizzle reads on the next `generate` stays intact.
-- The row-level security block below is HAND-ADDED and re-appended after that regeneration, for
-- the reason 0013 gave: drizzle can express columns, indexes and foreign keys, and cannot express
-- a policy. A new tenant table without a policy is not a missing feature — it is a table any
-- authenticated tenant can read in full, which is why the isolation suite asserts every one of
-- these by name rather than trusting that the block below stayed in step with the schema.
--
...
CREATE UNIQUE INDEX "suppliers_code_uq" ON "suppliers" USING btree ("org_id","code");
