-- RENUMBERED to 0020 by REGENERATION, not by renaming the file.
--
-- The catalog migration has now been renumbered twice: tickets 06/07 took 0015 and 0016, and on
-- this branch tickets 09, 12 and 13 hold 0017, 0018 and 0019. The journal and snapshot beside it
-- were produced by `drizzle-kit generate` at 0020; what follows is the migration's own
-- hand-authored SQL, re-applied on top — `generate` emits table DDL and nothing else, and the
-- eight row-level-security policies below are the point of the file.
--
-- Tickets 06 and 07 took 0015 and 0016 first, so this migration's number collided. The journal and
-- the snapshot beside it were produced by `drizzle-kit generate` at 0017; what follows is this
-- migration's own hand-authored SQL, re-applied on top — `generate` emits table DDL and nothing
-- else, and the row-level-security policies below are the point of the file.
--
