 ...
 *
 *  - Public demo (`STOPGAP_DEMO_MODE=on`) → PLAIN passthrough (`NextResponse.next()`), with NO
 *    Auth.js involved. The request is the anonymous read-only viewer; the demo's one explicit
 *    scenario action remains separately gated in the server action.
 *  - Auth configured, demo off → the Auth.js wrapper redirects an unauthenticated request to the
 *    Keycloak sign-in.
 *  - Auth NOT configured, demo off → fail closed with a 503. A deployment that forgot its IdP
 *    secret is not a public read-only demo just because the process can still render pages.
 *
...
const AUTH_NOT_CONFIGURED_MESSAGE =
  "authentication is not configured; set AUTH_SECRET and KEYCLOAK_CLIENT_SECRET or enable demo mode";

const middleware: NextMiddleware = (request, event) => {
  const env = getEnv();
  // The public demo is the only intentionally anonymous console surface. An unconfigured
  // non-demo process must not fall through to the viewer principal, because that would expose
  // tenant-scoped read data while making the deployment look healthy.
  if (env.STOPGAP_DEMO_MODE === "on") return NextResponse.next();
  if (!authConfigured(env)) {
    return NextResponse.json(
      { error: "authentication_not_configured", message: AUTH_NOT_CONFIGURED_MESSAGE },
...
export const config = {
  ...
  //    principal passes, and so does anyone when demo mode is on. An unconfigured non-demo
  //    deployment fails closed; an auth-configured deployment refuses an unauthenticated reader
  //    with 401 — HTML from `/docs`, JSON from `/openapi.json`.
  matcher: [
